...
public class CountryServiceImpl implements CountryService {
    ...
    private CountryDTO mapToCountryDTO(Optional<CountryEntity> countryEntity) {
        if (countryEntity.isPresent()) {
            ...
            countryDTO.setName(countryEntity.get().getCountryName());
            countryDTO.setId(countryEntity.get().getId());

            Set<CityDTO> cityDTOSet = countryEntity.get().getCities().stream().map(a -> new CityDTO(a.getId(), a.getCityName())).collect(Collectors.toSet());
    ...
    public void saveOrUpdate(CountryDTO countryDTO) {
        ...
        countryEntity.setCountryName(countryDTO.getName());
        ...
        if (countryEntity.getId() == null) {
            ...
            if (countryRepository.findById(countryEntity.getId()).isPresent()) {
                ...
                editCountryEntity.setCountryName(countryEntity.getCountryName());
    ...
    public List<CountryDTO> findAll() {
        ...
        return countryEntities.stream().map(a -> new CountryDTO(a.getId(), a.getCountryName())).collect(Collectors.toList());
    }
    ...
    public List<CountryDTO> findAll(Sort name) {
        ...
        for (CountryEntity countryEntity : countryEntityList) {
            countryDTO = new CountryDTO(countryEntity.getId(), countryEntity.getCountryName());

            Set<CityDTO> cityDTOSet = new HashSet<>();
            for (CityEntity cityEntity : countryEntity.getCities()) {
                cityDTO = new CityDTO(cityEntity.getId(), cityEntity.getCityName());

                List<CompanyDTO> companyDTOList = new ArrayList<>();
                for (CompanyEntity companyEntity : cityEntity.getCompanyEntity()) {
                    CompanyDTO companyDTO = new CompanyDTO(companyEntity.getId(), companyEntity.getCompanyName());
